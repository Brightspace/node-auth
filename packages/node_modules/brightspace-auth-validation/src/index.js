...
import jwkAllowedAlgorithms from 'jwk-allowed-algorithms';

import { decodeProtectedHeader as decodeHeader } from 'jose/decode/protected_header';
import { decodeJwt as decodePayload } from 'jose/jwt/decode';
import { compactVerify as verify } from 'jose/jws/compact/verify';

import AuthToken from 'brightspace-auth-token';

import { NoAuthorizationProvidedError, BadJsonWebTokenError, PublicKeyNotFoundError, PublicKeyLookupFailedError } from './errors.js';
...
const USER_AGENT = 'node-auth/brightspace-auth-validation/' + packageJson.version;
...
export default class AuthTokenValidator {
	constructor(opts) {
		opts = opts || {};
	...
	async fromHeaders(headers) {
		...
		if (!authHeader) {
			throw new NoAuthorizationProvidedError();
		}
	...
	async fromSignature(signature) {
		...
		await verifySignature(signature, token, publicKey);
	...
	_validateClaims(token) {
		...
		if ('undefined' !== typeof claims.exp) {
			...
			if ('number' !== typeof exp) {
				throw new BadJsonWebTokenError('Invalid "exp" claim');
			}

			const diff = now - exp;
			if (diff >= this._maxClockSkew) {
				throw new BadJsonWebTokenError(`Token expired (${diff} seconds)`);
			}
		}
		...
		if ('undefined' !== typeof claims.nbf) {
			...
			if ('number' !== typeof nbf) {
				throw new BadJsonWebTokenError('Invalid "nbf" claim');
			}

			const diff = now - nbf;

			if (diff < -1 * this._maxClockSkew) {
				throw new BadJsonWebTokenError(`Token not yet valid (${diff} seconds)`);
			}
		}
	...
	async _updatePublicKeysInner() {
		...
		if (!res.ok) {
			throw new PublicKeyLookupFailedError(new Error(`Request failed with status ${res.status}`));
		}
...
function decodeSignature(signature) {
	...
	let header = null;
	try {
		header = decodeHeader(signature);
	} catch (ignore) {
		throw new BadJsonWebTokenError('Not a valid JWT');
	}

	if ('string' !== typeof header.kid) {
		throw new BadJsonWebTokenError('Missing "kid" header');
	}

	if ('string' !== typeof header.alg) {
		throw new BadJsonWebTokenError('Missing "alg" header');
	}
...
async function verifySignature(signature, token, publicKey) {
	const alg = matchAlgorithm(publicKey, token);

	try {
		await verify(signature, publicKey.key, { algorithms: [alg] });
	} catch (e) {
		throw new BadJsonWebTokenError('Signature verification failed', e);
	}
}
...
function matchAlgorithm(publicKey, token) {
	...
	if (-1 === allowedAlgorithms.indexOf(requestedAlgorithm)) {
		throw new BadJsonWebTokenError('Token listed bad algorithm for key, "' + requestedAlgorithm + '"');
	}

...
import { InsufficientScopeError, InvalidContextError, ImpersonationRequiredError, ImpersonationNotAllowedError } from './errors.js';

export default class AssertionCompiler {
	constructor() {
		this._assertions = [];
	}
...
class ScopeAssertion extends AuthAssertion {
	...
	_assert(token) {
		...
		if (!matched) {
			throw new InsufficientScopeError(this.broad, this.narrow, this.permission);
		}
	}
...
class ContextAssertion extends AuthAssertion {
	...
	_assert(token) {
		...
		if (this._required) {
			...
			throw new InvalidContextError(this._required, this.context, token.context);
		}
	...
	_assert(token) {
		if (this._required) {
			if (!token.isImpersonating()) {
				throw new ImpersonationRequiredError();
			}
		} else if (/* this._rejected && */token.isImpersonating()) {
			throw new ImpersonationNotAllowedError(token.user, token.actualUser);
		}
	}
}

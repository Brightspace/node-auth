...
import keygen, { normalize } from '../src/rsa-key-generator.js';
...
describe('RSA', () => {
	describe('normalize', () => {
		...
		it('should throw TypeError when size in not an integer', () => {
			assert.throws(
				() => normalize({ signingKeySize: 2048.5 }),
				TypeError,
		...
		it('should return normalized, new, object when valid', () => {
			...
			const normal = normalize(inputOpts);
	...
	describe('keygen', /* @this */ function() {
		...
		it('should return the signing key as "signingKey" property', async() => {
			const key = await keygen(normalize({ signingKeySize: 2048 }), TEST_UUID);
			const signingKey = key.signingKey;
		...
		it('should return a public JWK as "jwk" property with provided kid', async() => {
			const key = await keygen(normalize({ signingKeySize: 2048 }), TEST_UUID);
			const jwk = key.jwk;

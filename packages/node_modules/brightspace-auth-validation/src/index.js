...
function loadJose() {
	if (!_joseImport) {
		_joseImport = Promise.all([
			import('jose/decode/protected_header'),
			import('jose/jwt/decode'),
			import('jose/jws/compact/verify'),
		]).then(([{ decodeProtectedHeader }, { decodeJwt }, { compactVerify }]) => ({
			decodeHeader: decodeProtectedHeader,
			decodePayload: decodeJwt,
...
class AuthTokenValidator {
	...
	async fromSignature(signature) {
		...
		const token = await decodeSignature(signature);
...
async function decodeSignature(signature) {
	...
	let header = null;
	try {
		header = jose.decodeHeader(signature);
	} catch (ignore) {
	...
	let payload = null;
	try {
		payload = jose.decodePayload(signature);
	} catch (ignore) {
		throw new errors.BadToken('Not a valid JWT');
	}
...
async function verifySignature(signature, token, publicKey) {
	...
	try {
		await jose.verify(signature, publicKey.key, { algorithms: [alg] });
	} catch (e) {
		throw new errors.BadToken('Signature verification failed', e);
	}
}

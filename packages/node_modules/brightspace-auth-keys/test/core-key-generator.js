...
import CoreKeyGenerator from '../src/core-key-generator.js';
import ecKeygen, { normalize as ecNormalize } from '../src/ec-key-generator.js';
import rsaKeygen, { normalize as rsaNormalize } from '../src/rsa-key-generator.js';

import DummyPublicKeyStore from './dummy-public-key-store.js';
...
describe('CoreKeyGenerator', () => {
	...
	describe('constructor(...)', () => {
		...
		describe('RSA', /* @this */ function() {
			...
			it('should check options with rsaNormalize and succeed', () => {
				sinon.spy(rsaKeygen, 'normalize');
				assert.strictEqual(0, rsaNormalize.callCount);
				createKeyGenerator({ signingKeyType: 'RSA', rsa: { singingKeySize: 2048 } });
				assert.strictEqual(1, rsaNormalize.callCount);
			});
		});

		describe('EC', () => {
			it('should check options with ecNormalize and succeed', () => {
				sinon.spy(ecKeygen, 'normalize');
				assert.strictEqual(0, ecNormalize.callCount);
				createKeyGenerator({ signingKeyType: 'EC', ec: { crv: 'P-256' } });
				assert.strictEqual(1, ecNormalize.callCount);
			});
		});

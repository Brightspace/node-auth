...
import BrightspaceAuthToken from 'brightspace-auth-token';

import AuthTokenValidator from '../src/index.js';
...
describe('validations', function() {
	...
	const jwk = Object.assign({}, publicKey, {
		kid: 'foo-bar-baz',
		use: 'sig'
	});
	...
	it('should throw "NoAuthorizationProvided" when there is no auth header', function() {
		return expect(validator.fromHeaders({}))
			.to.be.rejectedWith(errors.NoAuthorizationProvidedError);
	});
	...
	it('should throw "BadToken" when invalid token is sent', function() {
		return expect(validator.fromHeaders({ authorization: 'Bearer foobarbaz' }))
			.to.be.rejectedWith(errors.BadJsonWebTokenError, 'Not a valid JWT');
	});

	it('should throw "BadToken" when expired token is sent', async function() {
		token = await new SignJWT({})
			.setProtectedHeader({ alg: 'RS256', kid: jwk.kid })
			.setExpirationTime(Math.round(Date.now() / 1000) - maxClockSkew)
			.sign(privateKey);

		return expect(validator.fromHeaders({ authorization: `Bearer ${ token }` }))
			.to.be.rejectedWith(errors.BadJsonWebTokenError, /^Token expired/);
	});

	it('should throw "BadToken" when not-yet-valid token is sent (outside of skew)', async function() {
		token = await new SignJWT({})
			.setProtectedHeader({ alg: 'RS256', kid: jwk.kid })
			.setNotBefore(Math.round(Date.now() / 1000) + maxClockSkew + 1)
			.sign(privateKey);

		return expect(validator.fromHeaders({ authorization: `Bearer ${ token }` }))
			.to.be.rejectedWith(errors.BadJsonWebTokenError, /^Token not yet valid/);
	});
	...
	it('should throw "BadToken" for bad signature', async function() {
		...
		await expect(validator.fromHeaders({ authorization: `Bearer ${token}` }))
			.to.be.rejectedWith(errors.BadJsonWebTokenError, 'Signature verification failed');
	});

	it('should throw "PublicKeyNotFound" when no key with matching "kid" is found on auth server', async function() {
		token = await new SignJWT({})
			.setProtectedHeader({ alg: 'RS256', kid: 'errmegerd' })
			.sign(privateKey);
		...
		await expect(validator.fromHeaders({ authorization: `Bearer ${ token }` }))
			.to.be.rejectedWith(errors.PublicKeyNotFoundError);
	});
	...
	it('should NOT throw "PublicKeyLookupFailed" when there WAS error requesting the jwks', async function() {
		...
		const res = await validator.fromHeaders({
			authorization: `Bearer ${ token }`
		});
		expect(res).to.be.instanceof(BrightspaceAuthToken);
		expect(res.source).to.equal(token);
	});
	...
	it('should return BrightspaceAuthToken when matching "kid" is found on auth server, signature is valid, and expiry is within clock skew', async function() {
		token = await new SignJWT({ key: 'val' })
			.setProtectedHeader({ alg: 'RS256', kid: jwk.kid })
			.setExpirationTime(Math.round(Date.now() / 1000) - maxClockSkew + 1)
			.sign(privateKey);
	...
	it('should return BrightspaceAuthToken when matching "kid" is found on auth server, signature is valid and nbf is within clock skew', async function() {
		token = await new SignJWT({ key: 'val' })
			.setProtectedHeader({ alg: 'RS256', kid: jwk.kid })
			.setNotBefore(Math.round(Date.now() / 1000) + maxClockSkew)
			.sign(privateKey);
